...
const INV1: u128 = 0x7c6becf1e01faadd; // 2**256 //       lambda逆元的[64, 127] bit 
const INV0: u128 = 0x63f6e522f6cfee30; // 2**256 //       lambda逆元的[0, 63] bit
const MASK64: u128 = 0xffffffffffffffff; //将128位整数截断为低64位
...
pub fn decompose(scalar: &G1ScalarField, window_bits: u32) -> (G1ScalarField, G1ScalarField, bool, bool) {
    ...
    // scalar = q * lambda + r        k = k1 +λ*k2   quotient = k2  t = k1



    //q = scalar * lambda的逆,放入整数考虑  q =scalar/lambda 与r没关系
    //下面是为了计算scalar 乘以 lambda的逆的结果  分步计算  >>64:表示进位，防止精度丢失
    let q0: u128 = INV0 * s2;
    let q1: u128 = INV1 * s2 + INV0 * s3 + (q0 >> 64);
    let q2: u128 = s2 + INV1 * s3 + (q1 >> 64); //q2 = scalar * lambda的逆
    let q3: u128 = s3 + (q2 >> 64);

    let mut quotient0: u128 = q2 & MASK64; //q的高位
    let mut quotient1: u128 = q3 & MASK64; //q的低位




    // t = quotient * LAMBDA //分步计算t
    ...
    //loop 是为了平衡k1 k2之间的关系  k= k1 +lambda *k2 
    //r0 r1 -> k1的高低位  q0 q1 -> k2的高低位  
    loop {
    ...
    if 128 % window_bits == 0 {
        ...
        //经过loop以后，q0 q1 r0 r1可能为负，所以需要再次处理一下
    }
...
//底层蒙哥马利会用到这4个常量  曲线固定 常量也是固定

// use sP = (N - s)(-P) to make scalar smaller, which ensures scalar MSB is not set, and the decomposed phi has MSB unset
fn glv_preprocess_scalar(scalar: &G1ScalarField, window_bits: u32) -> (u128, u128, u128, u128, bool) {
    ...
    if 128 % window_bits == 0 {
        if s[3] >= 0x3FFFFFFFFFFFFFFF {
            //0x3FFFFFFFFFFFFFF表示2^62-1 最高位是为了劈一半 次高位表符号位 为了快速计算sp，如果s比较大，我们不直接算sP，而是计算(N-s)*P,二者等价
            is_neg_scalar = true;

            let mut carry: i128 = 0;
            carry = carry + R0 - s[0] as i128; //常量R0-R3 是预定义计算好的  Ri -s[i]为正
